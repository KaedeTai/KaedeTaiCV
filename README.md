# KAEDE TAI (戴志洋)

![Kaede Tai](https://raw.githubusercontent.com/KaedeTai/KaedeTaiCV/main/images/kaede_20260228.jpg)

**Contact Information**  
Tel: +886-910-094490  
LinkedIn: kaedetai  
Email: kaedetai@gmail.com  

---

## PROFESSIONAL SUMMARY

Serial entrepreneur and high-frequency quantitative trader with expertise in cybersecurity and technology infrastructure. Combines cross-disciplinary technical background with business acumen to transform innovative concepts into scalable, deployable products and systems. Passionate about leveraging technology and finance to create meaningful impact, driven by a commitment to making the world a better place through practical solutions.

From the invention of the computer and the rise of the Internet, to mobile commerce, blockchain and cryptocurrencies, and now AI reshaping the world, I have stayed on the front line as a hands-on builder and leader. My experience spans contributing to open-source projects as an individual, building teams from zero, and leading teams within large telecoms and technology companies. As delivery methodologies evolved—from waterfall and Agile to Lean Startup and today’s AI copilot-driven vibe coding—the only thing that has never changed is my commitment to building exceptional teams and guiding them through challenges to achieve ambitious goals.

---

## WORK EXPERIENCE

### **QTX Holdings** | CTO
*Dec 2024 - Nov 2025*  
- Founded and scaled quantitative trading infrastructure SaaS platform  
- Led fundraising initiatives and business development strategy  
- Built and managed cross-functional engineering teams for trading systems
- Binance Futures message received in 1.6 milliseconds (industry-leading low-latency)

### **Quantrend Technology** | CTO  
*Oct 2020 - Nov 2024*  
- Built a 20-person cross-functional team, establishing quantitative trading infrastructure  
- Developed AI-driven high-frequency trading systems with sub-millisecond latency  
- Directed business development, fundraising, and strategic partnerships across global markets  
- Established engineering culture, hiring processes, and performance management frameworks  
- Binance Futures Market Making 200B+ volume traded
- Achieved Binance VIP 9 status and Market Maker Tier 5

### **FiO Inc.** | CTO  
*Feb 2019 - Oct 2020*  
- Led development of IBM Hyperledger and IOTA blockchain solutions  
- Built Blockchain-as-a-Service platform from concept to deployment  
- Managed fundraising and team leadership for fintech startup

### **SELF TOKEN** | CTO  
*Nov 2018 - Feb 2019*  
- Designed and developed token-based payment platform architecture  
- Created ERC-777 smart contract systems for digital asset management  
- Served as technical advisor for film production "The Last Thieves"

### **KRONOS Research** | Lead Engineer  
*Apr 2018 - Nov 2018*  
- Designed high-frequency trading system architecture using C++  
- Implemented low-latency trading algorithms and execution systems

### **CTO.tw** | Founder & CEO  
*Aug 2015 - Apr 2018*  
- Founded CTO-as-a-Service consultancy for technology startups  
- Developed Fanily fan community platform for Dreamland Creative  
- Provided CTO services for customs logistics and clearance systems  
- Mentored startups through Taitung, Pingtung, and Tainan entrepreneurship programs  
- Served as NASA Space Apps Challenge mentor and TEDxTaipei board member

### **Yahoo! Inc.** | Lead Engineer  
*Dec 2013 - Aug 2015*  
- Managed global search website infrastructure and operations  
- Developed AI algorithms for search optimization and personalization

### **Richi Inc.** | CTO  
*May 2011 - Dec 2013*  
- Developed points exchange and loyalty program systems  
- Led fundraising efforts and implemented agile development workflows  
- Managed customer service and business development operations

### **Otiga Inc.** | Technical Director  
*Dec 2006 - May 2011*  
- Directed mobile WAP and application development projects  
- Managed development and operations of Universal Music official websites  
- Built and led technical teams across multiple product lines

### **Chunghwa Telecom (HiNet)** | Project Manager / System Administrator / Information Security Specialist  
*Jan 2003 - Dec 2006*  
- Served as network and information security administrator for Ministry of Transportation  
- Managed Directorate General of Highways Information Management System  
- Planned and implemented Environmental Protection Administration Recycling Management System

### **KKBOX Inc.** | Software Engineer / Project Manager  
*Dec 1999 - Jan 2003*  
- Developed dynamic websites using ASP and PHP technologies  
- Managed web application projects and deployment processes

### **Hewlett-Packard Taiwan** | Software Engineer / System Analyst  
*Jun 1996 - Jul 1997*  
- Developed Tzu Chi Hospital Medical Management System  
- Implemented solutions using Visual Basic and MS SQL Server

---

## EDUCATION

### **MBA in Information Management**  
*Sep 2000 - Jun 2002*  
National Taiwan University

### **B.S. in Information Management**  
*Sep 1994 - Jun 2000*  
National Taiwan University

---

## SKILLS & EXPERTISE

### **Quantitative Finance**
- High-frequency trading systems
- Market making and algorithmic trading
- Risk management and quantitative strategies

### **Blockchain & Cryptocurrency**
- Hyperledger and IOTA solutions
- ERC standards and smart contract development
- Token economics and digital asset management

### **Cybersecurity**
- Network security and information protection
- System administration and threat analysis
- Security architecture and compliance

### **Software Development**
- Rust, C++, Python, Solidity, JavaScript, C#
- NoSQL, MySQL, MS SQL database management
- Full-stack development and deployment

### **Artificial Intelligence**
- AI-driven trading systems
- Generative AI image pipelines
- Autonomous AI agent design with scheduled task automation

### **Special Education**
- Special education teacher focused on neurodivergent gifted autism spectrum/Asperger’s profile, ADHD, highly sensitive students
- Strength-based coaching for learning, executive function, emotional regulation, and social communication
- Individualized learning plans and close collaboration with families and educators

### **Entrepreneurship**
- Venture founding and fundraising
- Business strategy and startup mentoring
- Product-market fit and scaling

### **Leadership & Executive Management**
- Recruited, trained, and led cross-functional teams from inception to high-performance units
- Defined technology roadmaps, architectural decisions, and innovation pipelines
- Board-level reporting, investor relations, and strategic business planning
- Established engineering culture, career ladders, and performance management systems
- Oversaw multi-million dollar technology budgets and resource allocation
- Aligned technical execution with business objectives across investors, clients, and partners
- Led incident response, business continuity, and risk assessment frameworks
- Built leadership pipelines and mentored next-generation technical leaders

---

## ACHIEVEMENTS

- **2015**: Invited speaker at TEDxTaipei conference: https://www.youtube.com/watch?v=d00gxN0P0sI
- **1995**: Founded SOB BBS and created Water Ball, pioneering instant messaging service
- **1991**: Reverse-engineered DOS software copy protection systems

## LANGUAGES
- Mandarin Chinese (Native)
- English (Professional)

## INTERESTS
- International travel and photography
- Formula 1, baseball and basketball
- Music appreciation and performance
- Fitness and endurance training
